      ...
      <section className="gradient-hero text-primary-foreground py-16 md:py-24 text-center">
        <div className="container space-y-6 animate-fade-in">
          ...
          <p className="text-lg text-white/80 max-w-2xl mx-auto">
            Análise de paridade, soma e detalhamento geográfico dos últimos sorteios.
          </p>
          <Button
            asChild
            size="lg"
            className="gradient-accent shadow-glow transition-all active:scale-95"
          >
      ...
      <section className="py-12 container">
        ...
        {isLoading ? (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-3xl font-bold leading-none">
                        {pares}P <span className="text-slate-600">/</span> {impares}Í
                      </p>
              ...
              <Card className="border-none shadow-md bg-primary text-primary-foreground">
                <CardContent className="p-6">
                  ...
                  <div className="flex justify-between items-end">
                    ...
                    {estaAcumulado && (
                      <span className="text-[9px] bg-white/20 px-2 py-1 rounded font-bold mb-1">
                        ACUMULOU
                      </span>
                    )}
                  </div>
              ...
              <Card className="border-none shadow-sm bg-white overflow-hidden mt-6">
                ...
                <CardContent className="p-4">
                  <div className="flex flex-wrap gap-2">
                    {(c.listaMunicipioUFGanhadores || c.municipios || []).map(
                      (item: any, idx: number) => (
                        <div
                          key={idx}
                          className="flex items-center gap-2 bg-gray-50 border border-gray-100 px-3 py-2 rounded-xl text-sm hover:border-red-200 transition-colors"
                        >
                          <span className="font-black text-red-600">
                            {item.uf || item.siglaUF || "--"}
                          </span>
                          <span className="text-gray-400">|</span>
                          <span className="font-semibold text-gray-700">
                            {item.municipio || item.nomeMunicipio}
                          </span>
                          <span className="bg-red-100 text-red-700 text-[10px] px-2 py-0.5 rounded-full font-bold">
                            {item.ganhadores || item.quantidadeGanhadores || 1}{" "}
                            {(item.ganhadores || item.quantidadeGanhadores || 1) > 1
                              ? "apostas"
                              : "aposta"}
                          </span>
                        </div>
          ...
          <div className="text-center py-20 border rounded-3xl bg-muted/10">
            Não foi possível carregar os dados oficiais.
          </div>
        )}
      </section>
    </Layout>
